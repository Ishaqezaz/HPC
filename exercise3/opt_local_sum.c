...
double opt_local_sum(double *x, size_t size, int num_threads) {
    double local_sum[MAX_THREADS][PAD] = {{0.0}}; 
    ...
    #pragma omp parallel num_threads(num_threads)
    {
        ...
        for (size_t i = thread_num * num_elements_per_thread; i < (thread_num + 1) * num_elements_per_thread; i++) {
            sum += x[i];
        }

        local_sum[thread_num][0] = sum; 
    }

    double final_sum = 0.0;
    for (int i = 0; i < num_threads; i++) {
        final_sum += local_sum[i][0];
    }
...
int main() {
    ...
    for (int i = 0; i < num_thread_counts; i++) {
        ...
        for (int trial = 0; trial < num_trials; trial++) {
            ...
            double sum = opt_local_sum(array, array_size, num_threads);
